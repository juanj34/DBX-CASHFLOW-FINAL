...
import { OIInputs, OIYearlyProjection } from '../useOICalculations';
...
import { OIYearlyProjectionTable } from '../OIYearlyProjectionTable';
...
export const CompactRentCard = ({
  ...
  embedded = false,
  yearlyProjections,
  showAirbnbColumn = false,
  unitSizeSqf: propUnitSizeSqf,
  ...
  const content = (
    <>
      ...
      <div className={showAirbnbComparison ? "grid grid-cols-2" : ""}>
        ...
        {showAirbnbComparison && (
          <div className="p-3 space-y-1.5 border-l border-border bg-orange-500/5">
            ...
            {/* ST breakdown — collapsible like LT */}
            <Collapsible>
              <CollapsibleTrigger className="flex items-center gap-1 text-[10px] text-theme-text-muted hover:text-theme-text w-full justify-center py-1.5 transition-colors group">
                <span>View breakdown</span>
                <ChevronDown className="w-3 h-3 transition-transform duration-200 group-data-[state=open]:rotate-180" />
      ...
      {/* 7-Year Projection Table toggle */}
      {yearlyProjections && yearlyProjections.length > 0 && (
        <div className="border-t border-theme-border">
          <button
            onClick={() => setShowProjectionTable(!showProjectionTable)}
            className="w-full flex items-center justify-center gap-2 py-2.5 px-3 text-xs font-medium text-theme-accent hover:bg-theme-accent/5 transition-colors"
          >
            <Calendar className="w-3.5 h-3.5" />
            {showProjectionTable
              ? (t('hideProjectionTable') || 'Hide 7-Year Projection')
              : (t('showProjectionTable') || 'View 7-Year Projection')
            }
            <ChevronDown className={cn(
              "w-3 h-3 transition-transform duration-200",
              showProjectionTable && "rotate-180"
            )} />
          </button>
          {showProjectionTable && (
            <div className="px-3 pb-3">
              <OIYearlyProjectionTable
                ...
                currency={currency}
                rate={rate}
                showAirbnbComparison={showAirbnbColumn}
                unitSizeSqf={propUnitSizeSqf || unitSizeSqf}
                embedded
              />
            </div>
          )}
