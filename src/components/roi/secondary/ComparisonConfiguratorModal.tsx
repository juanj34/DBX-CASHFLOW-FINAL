...
interface ComparisonConfiguratorModalProps {
  ...
  onCompare: (quoteId: string, secondaryInputs: SecondaryInputs) => void;
...
export const ComparisonConfiguratorModal = ({
  ...
  const [step, setStep] = useState<1 | 2>(initialQuoteId ? 2 : 1);
  ...
  const handleCompare = () => {
    if (selectedQuote) {
      onCompare(selectedQuote.id, secondaryInputs);
      onOpenChange(false);
    }
  };

  const stepLabels = ['Seleccionar Quote', 'Configurar Secundaria'];
          ...
          <div className="flex items-center gap-3 pt-3">
            {[1, 2].map((s) => (
              <div key={s} className="flex items-center gap-2">
                ...
                <span className={`text-sm ${s === step ? 'text-theme-text font-medium' : 'text-theme-text-muted'}`}>
                  {stepLabels[s - 1]}
                </span>
                {s < 2 && <div className="w-8 h-px bg-theme-border" />}
              </div>
            ))}
          </div>
        </DialogHeader>
