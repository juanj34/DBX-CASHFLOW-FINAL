...
export const ExitsSection = ({ inputs, setInputs, currency }: ConfiguratorSectionProps) => {
  ...
  // All exits are now stored in customExits - no more separate auto-generated
  ...
  // Handover is always shown as reference point (not deletable)
  const handoverExit: ExitScenario = useMemo(() => ({
    id: 'handover',
    label: 'Handover',
  ...
  // All exits including handover
  const allExits = useMemo(() => {
    return [...exits, handoverExit].sort((a, b) => a.monthsFromBooking - b.monthsFromBooking);
  }, [exits, handoverExit]);
        ...
        <>
          ...
          <div className="flex items-center justify-between p-2 bg-theme-card/50 rounded-lg border border-theme-border/50">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-theme-accent">{exits.length} exits + handover</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {allExits.map((exit) => {
              ...
              const isHandover = exit.id === 'handover';
              
              return (
                <div
                  key={exit.id}
                  className={`p-3 rounded-xl border ${isHandover ? 'bg-blue-500/10 border-blue-500/30' : 'bg-theme-card border-theme-border'}`}
                >
                    ...
                    <div className="flex items-center gap-2">
                      {isEditing ? (
                        ...
                        <>
                          ...
                          {isHandover && (
                            <span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded">Handover</span>
                          )}
                        </>
                      )}
                    </div>
                    
                    {!isHandover && !isEditing && (
